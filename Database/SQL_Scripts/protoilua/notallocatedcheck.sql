...
SELECT sp.id, 
        ...
        sp.area, 
        sp.personLimit, 
        if(sp.area >= (SELECT area FROM Subject WHERE id = :subId), "Y", "N") as areaOk, 
        if(sp.personLimit >= (SELECT groupSize FROM Subject WHERE id = :subId), "Y", "N") as personLimitOk, 
        if(getMissingItemAmount(:subId ,sp.id) > 0, "Y", "N") AS "missingItems",
        sp.inUse 
    FROM `Space` sp;
...
CREATE OR REPLACE PROCEDURE checkSpace(subId INT)
SELECT sp.id, 
        sp.name, 
        sp.area, 
        sp.personLimit, 
